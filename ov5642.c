...
*
*/
static int ov5642_get_shutter(void)
{
	int shutter;
	unsigned char regval;
	shutter = (ov5642_read_reg(0x3500,&regval) & 0x0f);
	shutter = (shutter<<8) + ov5642_read_reg(0x3501,&regval);
	shutter = (shutter<<4) + (ov5642_read_reg(0x3502,&regval)>>4);
	
	return shutter;
}
static int ov5642_set_shutter(int shutter)
{
	int temp;
	...
	temp = shutter & 0x0f;
	temp = temp<<4;
	ov5642_write_reg(0x3502,temp);
	
	temp = shutter & 0xfff;
	temp = temp>>4;
	ov5642_write_reg(0x3501,temp);
	
	temp = shutter>>12;
	ov5642_write_reg(0x3500,temp);
	
	return 0;
}
static void ov5642_turn_on_AE_AG(int enable)
{
	unsigned char ae_ag_ctrl;

	ov5642_read_reg(0x3503,&ae_ag_ctrl);
	if(enable)
	{
	...
	ov5642_write_reg(0x3503,ae_ag_ctrl);
}
/*!
...
static int ioctl_g_ctrl(struct v4l2_int_device *s, struct v4l2_control *vc)
{
	...
	case V4L2_CID_EXPOSURE_AUTO:
			ov5642_read_reg(0x3503,&ae_ag_ctrl);	
			if((ae_ag_ctrl & 0x03)== 0x03) vc->value = 1;
			else vc->value = 0;	
		break;
	case V4L2_CID_EXPOSURE_ABSOLUTE:
			vc->value = ov5642_get_shutter();
		break;
	default:
		ret = -EINVAL;
	}
...
//		break;
	...
	case V4L2_CID_EXPOSURE_AUTO:
		...
		if(vc->value == V4L2_EXPOSURE_MANUAL)
		{
			ov5642_turn_on_AE_AG(0);	
		}
		else
		{	
			ov5642_turn_on_AE_AG(1);
		}
		break;
	case V4L2_CID_EXPOSURE_ABSOLUTE:
		oldshutter = ov5642_get_shutter();
		ov5642_set_shutter(vc->value);
		pr_debug("set ov5642 shutter from %u to %u\n",oldshutter,vc->value);
		break;
	default:
		retval = -EPERM;
		break;
